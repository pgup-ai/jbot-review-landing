...
/** Block-level containers whose children are emitted as blocks. */
...
/** Decode the HTML entities these pages use, plus any numeric reference. */
...
/** Split a tag's attribute string into a plain object. */
...
/**
 * Parse an HTML fragment into a tree of {tag, attrs, children} and string nodes.
 * Tolerates the unclosed `<p>`/`<li>` style these pages don't actually use, by
 * closing implicitly when an ancestor closes.
 */
...
/** True for nodes the reader should never see (decorative or hidden). */
...
/** Recover the plain text of a subtree, entities decoded, tags stripped. */
...
/** Render inline content (everything that lives inside a block). */
function renderInline(nodes, context) {
  ...
  for (const node of nodes) {
    ...
    if (hasClass(node, 'ftag') || hasClass(node, 'ptag') || hasClass(node, 'new-tag')) {
...
/** Render a list of nodes as Markdown blocks separated by blank lines. */
 ...
 * @param {{origin?: string, canonical?: string}} [context]
