...
Mostly because the model works one step at a time, and every step is a full round trip. When we traced J-Bot Review in production, two-thirds of the main review’s turns went to looking code up, and those turns took 61% of the model’s time. Running the tools themselves took 1.8% of the session. Nearly all the rest was the model deciding what to do next.

J-Bot Review is an open-source agentic PR reviewer that runs as a GitHub Action in your own CI, with the model you choose. We spent five weeks making it faster, and this series is the write-up. We went in expecting slow tools. What we found was a model doing lookups one at a time, plus a handful of bugs that quietly threw work away.
...
On the backend that reports tool time separately, file reads and searches took 1.8% of session time. In one full review we profiled, the tools ran for 196 milliseconds in total. The review took 323 seconds.

The rest is the model reading, reasoning and picking its next move, and you never see most of it. On one backend we audited, 94 to 99% of the output tokens were hidden reasoning. A typical focused pass produced 415 characters of visible answer out of 3,962 output tokens.
...
Two-thirds of all turns were nothing but lookups like these, and they ate 61% of the main review’s model time. Most of them could be predicted from the diff alone. That’s where the idea for context packs came from, and [Part 3](https://www.pgupai.com/guides/context-pack-ai-code-review) covers them.

### Size didn’t predict time

We assumed big pull requests were the slow ones. They weren’t. The correlation between a pull request’s size and its review time was 0.03, which is next to nothing. How much the model reasoned was a far better predictor, at 0.50.
...
Nothing we changed in the code moved the clock as much as which model a run landed on, and through which provider:

- Median main-review time was 54 seconds on one route and 1,060 seconds on another.
- On the same set of test pull requests, one free model finished in about 50 seconds and another took about 24 minutes.
- Dropping reasoning effort to low cut the median review from 63.6 to 37.7 seconds, a paired change of −29.8% that held in 21 of 24 pairs. On another model the same setting made it reason 36% more, so there’s no single right setting.

Speed alone can fool you, too. One model finished its main reviews in a median of 12 seconds, and 13 of its 14 reviews came back empty.

## What was quietly missing

The hardest problems to find didn’t raise an error. From the outside, each of these looked like a finished review:

- **Files the index couldn’t parse.** 1,282 of 4,014 TypeScript files failed on decorators, so services and controllers had no caller evidence at all. We found it during the context-pack work.
- **A reader that stopped at 48 KiB.** One backend’s file reader returned only the first 48 KiB of a file, with no way to page or search. As far as the model knew, a helper deep in a large file didn’t exist.
- **Confirmed findings that never posted.** A tentative finding kept its tentative label even after the verifier confirmed it, so the publishing step held it back. After the fix, a real defect was posted in 3 of 3 runs instead of 0 of 3.
- **Work that finished, then got thrown away.** Side passes still running 300 seconds after the main review were cut off and their work discarded, 345 to 450 seconds of model time each.
- **The one file nobody opened.** A serious bug slipped through because the unchanged file that called the changed code never made it into the review. No diff was truncated. The reviewer just never looked there.
- **Rules past the first 24 KB.** Guideline files were cut off at 24 KB, so long documents lost their second halves. [Part 4](https://www.pgupai.com/guides/ai-code-review-team-guidelines) is about that one.
- **Bookkeeping cut short.** We spotted this one in our own run logs. The steps that resolve fixed review threads and summarize new commits were stopped 9 seconds after they started, and the run then waited 7 more minutes for other passes anyway.
...
Our earlier speed tests had used small synthetic pull requests that finished in 20 to 60 seconds. They couldn’t show the two things that mattered most in production: which model route a run happened to draw, and how many attempts were wasted or cancelled. [Part 2](https://www.pgupai.com/guides/ai-code-review-speed-experiments) covers the 23 ideas we tried next.

## FAQ

### Why does AI code review take minutes when a chatbot answers in seconds?

A chat answer is usually one turn. A review is dozens. In our data a main review averaged about 15 turns in a row and 26 tool calls, and each turn waits for the model to read, reason and pick its next step. Several review passes run too, and the slowest one decides when the review is done.

### Does a bigger pull request take longer to review?

Not reliably. In our production data the correlation between pull-request size and review time was 0.03. How much the model reasoned was a much better predictor, at 0.50.
...
Both, in different ways. The model route sets the pace. Median main-review time ranged from 54 seconds to 1,060 seconds between routes. The reviewer’s design decides how many turns the model needs, and that’s the part context packs changed.
